        ...
        success: function (robe) {
               svaRoba=robe;
                robe.forEach(function (roba) {
                    $('#roba').append('<tr> <td style="display:none;">' + roba.id+ '</td><td>' + roba.naziv+ '</td> </tr>');
                });
            }
    });
}
...
$('#roba').on( 'click', 'tr', function () {
    var robaId = $(this).children(':first').text();
    $('#edit-robu').modal('toggle');

    var roba= svaRoba.find(function(element) {
            return element.id == robaId;
  });
    //console.log(sveGrupeRobe);

    var nazivRobe = $('#roba-naziv-edit').val(roba.naziv);
    var jedinicaMere = $('#roba-jedinica-mere-edit').val(roba.jedinicaMere); //tako nesto
